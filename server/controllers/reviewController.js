import { PropertyReview, HostReview, Booking, Property, User } from "../models/models.js"
...
class ReviewController{
    ...
    async createPropertyReview(req,res,next){
        try{
            const {bookingId, rating, comment, cleanliness, accuracy,
                communication, location, checkIn
            } = req.body
            ...
            if(!booking){
                return next(ApiError.badRequest("Бронирование не найдено или не принадлежит вам"))
            }
    ...
    async createHostReview(req,res,next){
        try{
            ...
            //Находим айди хоста
            const hostId = booking.Property.userId

            //Создание отзыва
            const review = await HostReview.create({
                bookingId, hostId, userId, rating, comment
            })

            //Обновление среднего рейтинга хозяина сразу после отзыва
